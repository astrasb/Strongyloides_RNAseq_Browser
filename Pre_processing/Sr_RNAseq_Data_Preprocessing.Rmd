...
This script checks the qualitiy of the fastq files and performs an alignment to the Strongyloides ratti cDNA transcriptome reference with Kallisto. To run this 'shell script' you will need to open your terminal and navigate to the directory where this script resides on your computer. For addditional instructions, see comments at head of script. 
...
This chunk of code generates and saves a digitial gene expression list that can be easily shared/loaded for downstream filtering/normalization. It generates a scatter plot of unfiltered and non-normalized transcripts per million data.    
...
write.csv(log2.cpm.filtered.norm, 
          file = file.path(output.path,
                           "SrRNAseq_log2cpm_filtered_norm.csv"))
