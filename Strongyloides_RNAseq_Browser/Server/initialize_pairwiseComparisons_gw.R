...
output$pairwiseSelector_GW <- renderUI({
    ...
    list(
        panel(
            ...
            heading = tagList(h5(
                shiny::icon("fas fa-sliders-h"),
                "Step 2: Pick Life Stage Comparisons"
            )),
            status = "primary",
            p(
                tags$em(
                    'Users may set up a single pairwise comparison for differential gene analysis using the selection menus below. Alternatively, users may also input multiple pairwise comparisons by typing a comma-separated list of contrasts into the textbox.',
                    style = "color: #7b8a8b"
                )
            ),
            ...
            selectInput(
                ...
                choices = c('Choose one or more' = ''
                            , as.list(
                                levels(vals$target.contrast.options)
                            )),
                selectize = TRUE,
                multiple = TRUE
            ),
            ...
            p(
                tags$em(
                    'If using the textbox to type comma-separated contrasts, please use the format: (Target)-(Contrast). For example typing "iL3-PF, (iL3+iL3a)-(PF+FLF)" will run two pairwise comparisons: iL3 vs PF and iL3+iL3a vs PF+FLF. To correct for multiple, closely related pairwise comparisons, use the toggle switch below.',
                    style = "color: #7b8a8b"
                )
            ),
...
parse_contrasts_GW <- eventReactive(input$goLifeStage_GW, {
    ...
    validate(need((
        isTruthy(input$multiContrasts_GW) ||
            (
                isTruthy(input$selectTarget_GW) &&
                    isTruthy(input$selectContrast_GW)
            )
    ),
...
output$contrastDisplaySelection_GW <- renderUI({
    ...
    isolate({
        panel(
            heading = tagList(h5(
                shiny::icon("fas fa-filter"),
                "Pick Contrast to Display"
            )),
            status = "default",
            pickerInput(
                "displayedComparison_GW",
                NULL,
                comparison,
