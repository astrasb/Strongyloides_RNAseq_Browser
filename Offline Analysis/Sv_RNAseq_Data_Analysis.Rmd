---
title: Offline Data Analysis of Strongyloides venezuelensis bulk RNAseq data
output:
  html_document:
    code_folding: hide
    df_print: paged
...
# Introduction
The goal of this file is to lay out proof-of-concept analyses of the *Strongyloides venezuelensis* RNAseq dataset originally published by [Hunt *et al* 2018](https://www.nature.com/articles/s41598-018-23514-z).  
...
Here, only the first group (FLF_PF) is analyzed, as the lack of biological replicates in Group iL3_extended requires a significant adjustment to the analysis pipeline. Specifically, the use of empirical bayes smoothing of gene-wise standard deviations to [provide increased power](https://www.degruyter.com/doi/10.2202/1544-6115.1027) is not possible without biological replicates. 

### Data Pre-Processing    
A full description of Kallisto alignment and data filtering/normalization steps can be found in `Sv_RNAseq_Data_Processing.rmd`.
...
load(file = "../Strongyloides_RNAseq_Browser/Data/Sv_vDGEList")
...
ggsave("Sv_Multivariate_Plots_PCA.pdf",
       plot = p3,
       device = "pdf",
       height = 4,
       #width = 7,
...
ggsave("Sv_Multivariate_Plots_Small_Multiples.pdf",
       plot = p6,
       device = "pdf",
       height = 4,
       width = 8,
...
temp.dat <-  read.xlsx ("../Benchmarking/41598_2018_23514_MOESM2_ESM.xlsx", 
                        sheet = 1, startRow = 4)
...
ggplot(plotting.all, aes(x = logFC.x, y = logFC.y)) +
  ...
  labs(title = "S. venezuelensis: All Overlapping Genes: Str-Browser vs Hunt Data",
...
suppressPackageStartupMessages({
  ...
  library(egg)
  library(dendextend)
  source("../Strongyloides_RNAseq_Browser/Server/ggheatmap_local.R")
})
...
load(file = "../Outputs/Sv_allSamples_vDEGList")
...
```{r multivariate.nocorrection.1, echo = FALSE}
# Introduction to this chunk -----------
# This code chunk starts with filtered and normalized abundance data in a data frame (not tidy).
# It will implement hierarchical clustering and PCA analyses on the data.
# It will plot various graphs and can save them in PDF files.
...
```{r multivariate.nocorrection.2 , echo = FALSE}
# Principal component analysis (PCA) -------------
# this also works on a data matrix, not a data frame
pca.res <- prcomp(t(log2.cpm.filtered.norm), scale.=F, retx=T)
...
```{r multivariate.batch.corrected.1, echo = FALSE}
# Introduction to this chunk -----------
...
```{r multivariate.batch.corrected.2, echo = FALSE}
# Principal component analysis (PCA) -------------
# this also works on a data matrix, not a data frame
