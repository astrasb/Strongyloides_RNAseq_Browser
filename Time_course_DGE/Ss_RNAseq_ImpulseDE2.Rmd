...
# Introduction
As an alternative to limma-voom based pairwise differential gene analysis, here we analyze the Stolzfus bulk RNAseq dataset using differential expression alogrithms for longitudinal count data.
...
### Data Pre-Processing  
Kallisto and custom R scripts were used to perform ultra-fast read mapping of raw reads to the *S. stercoalis* reference transcriptome (PRJEB528.WBPS14.mRNA_transcripts, downloaded from [WormBase Parasite](https://parasite.wormbase.org/Strongyloides_stercoralis_prjeb528/Info/Index/) on 16 June 2020).   

Kallisto alignments are imported into the R environment and annotated with information imported via the Wormbase ParaSite BioMaRT. Annotation information includes: *C. elegans* homologs/percent homology, *S. ratti* homologs/percent homology, UniProtKB number, Interpro terms, GO terms, and general Description information. Annotation information is saved as an R object that is passed to a Shiny Web App for downstream browsing and on-demand analysis.  

Raw reads (unfiltered and non-normalized) are used to generate a `DESeqDataSet object`, which is used to generate a matric of integer count data. This matrix is passed into `ImpulseDE2`, for trajectory analysis. Also passed into `ImpulseDE2` is a design file that specifies the time points associated with each sample. Note: the numerical time values affect the curve fitting processes within `ImpulseDE2` such that it appears the most robust way to temporally code the set of life stages {FLF, iL3, iL3a, PF} is as values 0 - 4 (rather than calculating the amount of time that passes between life stages). Here, `ImpulseDE2` is run with the following parameters: no correction for batch, case-only analysis, and searching for transient expression.  

# Import Kallisto reads into R  
This process uses a different project design than the primary data pre-processing script. The ImpulseDE2 design file includes temporal assignments for each sample. Depending on the version of the design file, only a subset of samples from PRJEB3116 might be included. Kallisto reads are imported using different `tximport` parameters - here, abundance values are imported as is whereas the pre-processing script calculates counts from abundance using a `lengthScaledTPM` process that generates estimated counts that are scaled by average transcript length and then library size. In this code, those normalization steps occur in subsequent chunks.  
...
dfAnnotation <- read_tsv("../Data/S_stercoralis/Study_Design/PRJEB3116_ImpulseDE2_all_stages_design.txt",
                         col_types = "ccncc",
                         na = c("", "NA", "na"))
...
Generate annotated lists of genes displaying different trajectory profiles. ImpulseDE2 can identify 4 life-varying expression profiles: transient upregulation, transient downregulation, permanent upregulation, and permanent downregulation.  
...
```{r genes.of.interest.1, cache = FALSE, echo = FALSE}
suppressPackageStartupMessages({
  library(pheatmap)
...
```{r genes.of.interest.2, cache = FALSE, echo = FALSE}
...
goi.trajectory.results.df[,1:19] %>%
...
Create and annotated heatmap (using `ComplexHeatmap` library) that shows the expression of the gene set.  
...
goi.matrix <- goi.trajectory.results.df[,3:9] %>%
  as.matrix()
