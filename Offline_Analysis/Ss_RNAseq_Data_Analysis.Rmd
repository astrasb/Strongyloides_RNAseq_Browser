---
title: Offline Data Analysis of *Strongyloides stercoralis* bulk RNAseq data
output:
  html_document:
    code_folding: hide
    df_print: paged
...
myscores.Top10.interactive <- myscores.Top10 %>%
  DT::datatable(extensions = c('KeyTable', "FixedHeader", "Buttons", "RowGroup"),
                ...
                caption = htmltools::tags$caption(
                  ...
                  htmltools::tags$b('Top 10% of Genes Contributing to S. stercoralis PC1 and PC2')),
...
## Chemoreceptor Genes  
This section examines gene expression patterns in chemoreceptor genes identified by the Zamanian lab (Weeler *et al* 2020).
```{r CR.load, eval=FALSE, include=FALSE}

temp <- c('../Data/S_stercoralis/Ss_chemoreceptors.csv')

GoI <- suppressWarnings(read.csv(temp, 
                                 header = TRUE, 
                                 colClasses = "character", 
                                 strip.white = T)) %>%
  as_tibble() %>%
  dplyr::mutate(geneID = str_remove_all(Transcript_ID, "\\.[0-9]$")) %>%
  dplyr::mutate(geneID = str_remove_all(geneID, "[a-c]$")) %>%
  dplyr::group_by(geneID) %>%
  dplyr::slice_max(n = 1, order_by = str_length(`Sequence`), 
                   with_ties = FALSE) %>%
  dplyr::select(geneID)
...
### Heatmap of chemoreceptor gene expression  
```{r CR.heatmap, eval=FALSE, include=FALSE}
# Filter all gene expression levels, taking just those in the list of chemoreceptors
subset.diffGenes <- v.DEGList.filtered.norm$E %>%
  as_tibble(rownames = "geneID", .name_repair = "unique") %>%
  dplyr::filter(geneID %in% GoI$geneID)

rownames <- subset.diffGenes$geneID
subset.diffGenes <- subset.diffGenes %>%
  dplyr::select(!geneID) %>%
  as.matrix() 
rownames(subset.diffGenes) <- rownames
colnames(subset.diffGenes) <- paste0(v.DEGList.filtered.norm$targets$group,
                                     "...",
                                     substr(v.DEGList.filtered.norm$targets$samples, 
                                            nchar(
                                              as.character(v.DEGList.filtered.norm$targets$samples[1]))-2, nchar(
                                                as.character(v.DEGList.filtered.norm$targets$samples[1])))
)
