...
# PC1 elements (FLF, PF vs iL3s, iL3+, ppL1, ppL3, pfL1) and PC2 elements
...
comparison <- c('(PF+FLF)-(iL3+iL3a+pfL1+ppL1+ppL3)',
                '(PF+iL3+iL3a)-(FLF+pfL1+ppL1+ppL3)')
...
# PC2 Volcano Plot and Interactive Table ----
vplot2 <- ggplot(list.myTopHits.df[[2]]) +
  aes(y=-log10(BH.adj.P.Val), x=logFC, 
      text = paste(geneID, "<br>",
                   "logFC:", round(logFC, digits = 2), "<br>",
                   "p-val:", format(BH.adj.P.Val, digits = 3, scientific = TRUE))) +
  geom_point(size=2) +
  geom_hline(yintercept = -log10(adj.P.thresh), 
