...
p6<-ggplot(pca.pivot) +
  ...
  scale_fill_brewer(palette = "Set2") +
  facet_wrap(~PC) +
  geom_bar(data = PC1, stat = "identity") +
  geom_bar(data = PC2, stat = "identity") +
...
myscores.df <- pca.res$rotation[,1:3] %>% 
...
myscores.Top10 <- myscores.df %>%
  ...
  pivot_wider(id_cols = geneID,
              names_from = PC,
              values_from = scores) %>%
  dplyr::mutate(PC1_id = case_when(PC1 > 0 ~ "Parasitic",
                                   PC1 < 0 ~ "Free-living",
                                   is.na(PC1) ~ "--")) %>%
  dplyr::mutate(PC2_id = case_when(PC2 > 0 ~ "GroupC",
                                   PC2 < 0 ~ "GroupD",
                                   is.na(PC2) ~ "--")) %>%
...
suppressPackageStartupMessages({
  ...
  library(egg)
  library(dendextend)
  source("../Stercoralis_RNAseq_Browser/Server/ggheatmap_local.R")
})
