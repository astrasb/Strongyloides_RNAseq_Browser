...
# Generate comparison matrix for all unique pairwise comparisons ----
...
# Generates comparisons for all pairwise comparisons
...
save(output.DEG.SsRNAseq)

# Join and save the output lists of DEG Identificationfunctions
...
DEGs.only<- sapply(contrasts, function(x) {output.DEG.SsRNAseq[[x]]$diffGenes.TopHits}, USE.NAMES = TRUE, simplify = F)
TopHit.tbl <-sapply(contrasts, function(x) {output.DEG.SsRNAseq[[x]]$avg.myTopHits.df}, USE.NAMES = TRUE, simplify = F)
output.SsRNAseq<-Map(c, output.DEG.SsRNAseq)

save(DEGs.only,
     file = "./Outputs/SsLifeStageComparisons_subset")
save(TopHit.tbl,
     file = "./Outputs/SsLifeStageComparisons_TopHits")
```
